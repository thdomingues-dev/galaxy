    ...
    <div className="w-4/5 m-auto">
      <div className="h-screen flex flex-col justify-center items-center">
        <section className="bg-white w-3/4 rounded-lg flex justify-start items-center mb-8">
          <Image className="rounded-lg" src={`https://starwars-visualguide.com/assets/img/characters/${imgIndex}.jpg`} height={300} width={225} alt={`image${imgIndex}`} />
          <div className="ml-4">
            ...
            <ul className="text-left flex">
              {
                character.films.map((film, index) => 
                    <li key={index}>
                      <Image className="rounded-full" src={`https://starwars-visualguide.com/assets/img/films/${film.id}.jpg`} height={64} width={64} alt={film.title} />
                      {film.title}
                    </li>
                ) 
              }
            ...
            <ul className="text-left flex">
              {
                character.starships.map((starship, index) => 
                    <li key={index}>
                      <Image className="rounded-full" src={`https://starwars-visualguide.com/assets/img/starships/${starship.id}.jpg`} height={64} width={64} alt={starship.name} />
                      {starship.name}
                    </li>
                ) 
              }
            ...
            <ul className="text-left flex">
              {
                character.vehicles.map((vehicle, index) => 
                    <li key={index}>
                      <Image className="rounded-full" src={`https://starwars-visualguide.com/assets/img/vehicles/${vehicle.id}.jpg`} height={64} width={64} alt={vehicle.name} />
                      {vehicle.name}
                    </li>
                ) 
              }
...
export const getStaticProps: GetStaticProps = async (context) => {
  ...
  const films = await Promise.all(filmsIndex.map(async (filmId) => {
  ...
  const starships = await Promise.all(starshipsIndex.map(async (starshipId) => {
  ...
  const vehicles = await Promise.all(vehiclesIndex.map(async (vehiclesId) => {
